...
public class GradeBook {
    ...
    public GradeBook(String name, int gradesArray[][]) {
        ...
        setGrades(gradesArray); // store grades
    ...
    public void setGrades(int[][] someGrades) {
        someGrades = new int[grades.length][grades.length];
        for (int row = 0; row < someGrades.length; row++) {
            for (int column = 0; column < someGrades[row].length; column++) {
                this.grades[row][column] = someGrades[row][column];
            }
        }
    }
    ...
    public double[] getAllAverage() {
        int averageScore[] = new int[grades.length];
        double[] getAllGrades = new double[grades.length];

        for (int i = 0; i < getAllGrades.length; i++) {
            getAllGrades[i] = getAverage(averageScore);
        }
        return getAllGrades; // inicilalizirane na vseki element na dadeniq masiv
        // s sredna stoinost na student s zadadenata daljina ;
    }

    public double[] filterAverages(double[] averages, double aboveLevel) {
        int averageScore[] = new int[grades.length];
        double[] getAllGrades = new double[grades.length];
        ...
        for (int i = 0; i < getAllGrades.length; i++) {
            ...
            for (int j = 0; j < averages.length; j++) {
                if (getAllGrades[i] > aboveLevel) {
                    averages[j] = getAllGrades[i];
                }
            }
        }
        return averages;
